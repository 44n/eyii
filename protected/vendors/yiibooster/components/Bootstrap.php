 ...
 * @version 1.0.0
 */
defined('BOOTSTRAP_DEBUG') or define('BOOTSTRAP_DEBUG', false);
...
class Bootstrap extends CApplicationComponent
{
	...
	{
		$this->registerAssetCss('bootstrap' . (!BOOTSTRAP_DEBUG ? '.min' : '') . '.css');
	}
	...
	public function registerResponsiveCss()
	{
		...
		$cs->registerCssFile($this->getAssetsUrl() . '/css/bootstrap-responsive' . (!BOOTSTRAP_DEBUG ? '.min' : '') . '.css');
	}
	...
	public function registerJS($position = CClientScript::POS_HEAD)
	{
		...
		$cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap' . (!BOOTSTRAP_DEBUG ? '.min' : '') . '.js', $position);
	}
	...
	public function getAssetsUrl()
	{
		...
		else
		{
			...
			$assetsUrl = Yii::app()->assetManager->publish($assetsPath, false, -1, BOOTSTRAP_DEBUG);
